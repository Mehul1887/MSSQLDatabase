...
-- Description:	<Description,,DeleteSubscriptionMaster>
-- Call SP    :	DeleteSubscriptionMaster
-- =============================================
CREATE PROCEDURE [nan].[DeleteSubscriptionMaster]
    @IdList NVARCHAR(MAX) ,
    @DeletedBy BIGINT ,
    @PageId BIGINT
AS
	...
	SET NOCOUNT ON
        ...
        WHILE @Start <= @Total
            BEGIN
                ...
                FROM    @Tbl
                WHERE   Id = @Start;
                EXEC nan.IsReferenceExists N'SubscriptionMaster', @Id,
                    @IsUsed OUTPUT;
                IF @IsUsed = 0
                    BEGIN
                        UPDATE  SM
                        ...
                        VALUES  ( @DeletedBy ,
                                  @PageId ,
                                  'Delete record in table SubscriptionMaster' ,
                                  'SubscriptionMaster' ,
                                  @Id ,
                                  GETUTCDATE() ,
                                  @DeletedBy ,
